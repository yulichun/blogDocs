...
本手册描述了如何计算prometheus的资源的使用量，帮助客户合理规划promehtues的资源配置。

涉及的资源有内存、磁盘、带宽。注意，此处建议CPU使用大于等于4核。

## 5.样例服务的每次采集样本数

以下为样例服务的每次采集样本数，误差在10以内。
...
3. Unique Label Pairs* 它的数目是估算值，DDS服务、KKS服务、MMS服务的标签对大概50左右，网关的标签对和证书数、代理服务数、后端服务数有关。<br>
...
某证券上海机房中有50台“KKSGW+KKS”节点，现场10台“DDS+MMS”节点，KKSGW配置了10个代理服务，这些代理服务后端一共30个后端服务，证书20个，采集周期为20s。

```
# KKSGW每次采集样本数 = 4 * 证书数 + 14 * 代理数 + 10 * 后端服务数
KKSGW每次采集样本数 = 4 * 20 + 14 * 10 + 10 * 30 = 520

# 时间序列总数 = KKSGW每次采集样本数 * KKSGW数 + node-exporter每次样本数 * node-exporter数 + KKS每次样本数 * KKS服务数 + DDS每次样本数 * DDS服务数 + MMS每次样本数 * MMS服务数
...
某证券上海机房中有50台“KKSGW+KKS”节点，现场10台“DDS+MMS”节点，KKSGW配置了10个代理服务，这些代理服务后端一共30个后端服务，证书20个，采集周期为20s,采集周期为30天。
...
|-------|----------------------|----------------------|------------------|
|  DDS   |    50KB      |   2.64KB       |     19      |
|  MMS   |    21KB      |   1.31KB       |     15.6      |
|  KKS   |    21.2KB      |   1.33KB       |     15.9      |
|  网关   |    80KB      |   9.15KB      |     8.74      |
|  node-exporter   |    75KB      |   12.7KB      |     5.9      |

以上网络流量大小可以直接作为我们计算带宽使用量的参数，这是因为：DDS、MMS、KKS、node-exporter的样本数是几乎不变的，其次网关的样本数在变，但在样本变大的时候，实际指标还是那几个，文本内容是高度雷同的，在gzip压缩算法下可以忽略高度雷同内容的增加。

```
每次采集服务网络流量总和 = DDS数 * 2.64 + MMS数 * 1.31 + KKS数 * 1.33 + 网关数 * 9.15 + 节点数 * 12.7
```
...
```
# 每次采集服务网络流量总和 = DDS数 * 2.64 + MMS数 * 2.61 + KKS数 * 1.33 + 网关数 * 9.15 + 节点数 * 12.7
